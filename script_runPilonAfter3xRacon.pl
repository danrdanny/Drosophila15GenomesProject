...
foreach my $newStockName (sort keys %stocksToAlign) {
	...
	my $targetDir = "$pwd/polish_pass_pilon-after-raconx3";
	my $scratch = "/scratch/dem/raconx3";
	#my $scratch = $targetDir;
	my $logFile = "$targetDir/$newStockName.log";

	my $finalFasta = "$newStockName.$readType.minimap2.racon.x3.pilon.x$maxRuns.fasta";
	my $finalBam   = "$newStockName.$readType.minimap2.racon.x3.pilon.x$maxRuns.bam";
	my $finalVCF   = "$newStockName.$readType.minimap2.racon.x3.pilon.x$maxRuns.vcf.gz";
	...
	foreach my $pilonRunNum (1..$maxRuns) {
		...
		my $lastBam       = "$newStockName.$readType.minimap2.racon.x3.pilon.x$lastPilonRunNum.bam";
		my $lastVCF 	  = "$newStockName.$readType.minimap2.racon.x3.pilon.x$lastPilonRunNum.vcf.gz";
		my $lastFasta     = "$newStockName.$readType.minimap2.racon.x3.pilon.x$lastPilonRunNum.fasta";
		my $newBamPrefix  = "$newStockName.$readType.minimap2.racon.x3.pilon.x$pilonRunNum";
		my $newBam        = "$newStockName.$readType.minimap2.racon.x3.pilon.x$pilonRunNum.bam";
		my $newFasta	  = "$newStockName.$readType.minimap2.racon.x3.pilon.x$pilonRunNum.fasta";
		my $newVCF 	  = "$newStockName.$readType.minimap2.racon.x3.pilon.x$pilonRunNum.vcf.gz";

		if ($pilonRunNum == 1) {
			$lastBam   = "$newStockName.$readType.minimap2.racon.x3.bam";
			$lastVCF   = "$newStockName.$readType.minimap2.racon.x3.vcf.gz";
			$lastFasta = "$newStockName.$readType.minimap2.racon.x3.fasta";
		}
		...
		if (!-e "$targetDir/$lastBam") {
			...
			executeCommand("bwa mem -t $threads $targetDir/$lastFasta '<cat $f_reads' '<cat $r_reads' > $scratch/$newStockName.sam 2>/dev/null",$logFile);
			print "[".localtime(time)."] $newStockName | Samtools view.\n";
			executeCommand("samtools view -bS $scratch/$newStockName.sam > $scratch/$newStockName.bam 2>/dev/null",$logFile);
			print "[".localtime(time)."] $newStockName | Sorting $newStockName.bam.\n";
			executeCommand("samtools sort -\@ $threads $scratch/$newStockName.bam -o $scratch/$newStockName.sorted 2>/dev/null",$logFile); #creates x.sorted.bam
			executeCommand("mv $scratch/$newStockName.sorted $targetDir/$lastBam",$logFile);
			executeCommand("rm -f $scratch/$newStockName.bai $scratch/$newStockName.bam $scratch/$newStockName.sam",$logFile);
		...
		if (!-e "$targetDir/$newFasta") {
			...
			executeCommand("java -Xmx250G -jar /home/dem/bin/pilon-1.22.jar --threads $threads --genome $targetDir/$lastFasta --frags $targetDir/$lastBam --output $targetDir/$newBamPrefix --changes --diploid",$logFile);
		...
		if (!-e "$targetDir/$lastVCF") {
			print "[".localtime(time)."] $newStockName | Forking to make $lastVCF from $lastBam.\n";
			my $pid = fork();
			die "unable to fork: $!" unless defined($pid);
			if (!$pid) {
				print "[".localtime(time)."] $newStockName | Forked to $pid and calling SNPs on $lastBam\n";
				executeCommand("samtools mpileup -ugf $targetDir/$lastFasta $targetDir/$lastBam | bcftools call -vmO z -o $targetDir/$lastVCF 2>/dev/null",$logFile);
			...
			print "[".localtime(time)."] $newStockName | $lastVCF exists, skipping SNP calling.\n";
		}
	}
	...
	if (!-e "$targetDir/$finalBam") {
		...
		executeCommand("bwa mem -t $threads $targetDir/$finalFasta '<cat $f_reads' '<cat $r_reads' > $scratch/$newStockName.sam 2>/dev/null",$logFile);
		...
		executeCommand("samtools sort -\@ $threads $scratch/$newStockName.bam -o $scratch/$newStockName.sorted 2>/dev/null",$logFile);
		executeCommand("mv $scratch/$newStockName.sorted $targetDir/$finalBam",$logFile);
		executeCommand("rm -f $scratch/$newStockName.bai $scratch/$newStockName.bam $scratch/$newStockName.sam",$logFile);
